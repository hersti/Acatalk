...
export default function CourseActiveContributorsCard({
  ...
  return (
    <Card className="p-3 border shadow-sm">
      ...
      {loading ? (
        <p className="text-[11px] text-muted-foreground">Ders katkicilari yukleniyor...</p>
      ) : hasError ? (
        <div className="space-y-2">
          <p className="text-[11px] text-muted-foreground">Katki sinyalleri su an alinmiyor.</p>
        ...
        <p className="text-[11px] text-muted-foreground">
          Bu derste henuz one cikan katki yok. Ilk not, tartisma veya sohbet mesaji ile alan canlanir.
        </p>
      ) : (
        <div className="space-y-1.5">
          {contributors.map((contributor, index) => {
            const displayName = contributor.display_name || contributor.username || "Kullanici";
              ...
              <div key={contributor.user_id} className="rounded-md border border-border/60 bg-secondary/20 px-2 py-2">
                <div className="flex items-start gap-2">
                  <Avatar className="h-7 w-7 shrink-0">
                    ...
                    <AvatarFallback className="text-[10px] font-bold bg-primary/10 text-primary">{firstLetter}</AvatarFallback>
                  </Avatar>

                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      <Link to={`/user/${contributor.user_id}`} className="text-[11px] font-semibold truncate hover:text-primary transition-colors">
                        {displayName}
                      </Link>
                      <span className="text-[9px] text-muted-foreground">#{index + 1}</span>
                    </div>
                    <p className="text-[10px] text-muted-foreground truncate">@{contributor.username}</p>
                    <div className="mt-1 flex items-center gap-2 text-[10px] text-muted-foreground">
                      <span className="inline-flex items-center gap-1">
                        <Award className="h-2.5 w-2.5" /> {formatScore(contributor.course_reputation_v1)}
                      </span>
                      <span>{contributor.posts_30d + contributor.comments_30d} katki</span>
          ...
          <p className="text-[10px] text-muted-foreground pt-1 flex items-center gap-1">
            ...
            Siralama son 30 gun aktivite + kalite sinyaline gore hesaplanir.
          </p>
        </div>
      )}
    </Card>
