...
const typeConfig: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  ...
  past_exams: { label: "Cikmis", icon: ClipboardList, color: "bg-primary/10 text-primary" },
  discussion: { label: "Tartisma", icon: MessageSquare, color: "bg-accent/10 text-accent" },
    ...
    <Card className="p-3 border shadow-sm">
      ...
      {items.length === 0 ? (
        <p className="text-[11px] text-muted-foreground">
          Henuz yeni icerik yok. Ilk not, cikmis soru veya kaynak paylasimi bu alani canlandirir.
        </p>
      ) : (
