-- Feed 2.0 foundation: user course visits + feed snapshot RPCs (additive)

CREATE TABLE IF NOT EXISTS public.user_course_visits (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  course_id uuid NOT NULL REFERENCES public.courses(id) ON DELETE CASCADE,
  last_visited_at timestamptz NOT NULL DEFAULT now(),
  visit_count integer NOT NULL DEFAULT 1 CHECK (visit_count >= 1),
  last_source text NOT NULL DEFAULT 'course',
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE(user_id, course_id)
);

CREATE INDEX IF NOT EXISTS idx_user_course_visits_user_last_visited
  ON public.user_course_visits(user_id, last_visited_at DESC);

CREATE INDEX IF NOT EXISTS idx_courses_university_department_year
  ON public.courses(university, department, year);

CREATE INDEX IF NOT EXISTS idx_posts_status_course_created
  ON public.posts(status, course_id, created_at DESC);
...
DROP TRIGGER IF EXISTS trg_user_course_visits_updated_at ON public.user_course_visits;
CREATE TRIGGER trg_user_course_visits_updated_at
BEFORE UPDATE ON public.user_course_visits
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

ALTER TABLE public.user_course_visits ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view own course visits" ON public.user_course_visits;
CREATE POLICY "Users can view own course visits"
  ON public.user_course_visits
  FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own course visits" ON public.user_course_visits;
CREATE POLICY "Users can insert own course visits"
  ON public.user_course_visits
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own course visits" ON public.user_course_visits;
CREATE POLICY "Users can update own course visits"
  ON public.user_course_visits
  FOR UPDATE
  TO authenticated
  USING (auth.uid() = user_id)
...
DROP POLICY IF EXISTS "Users can delete own course visits" ON public.user_course_visits;
CREATE POLICY "Users can delete own course visits"
  ON public.user_course_visits
  FOR DELETE
  TO authenticated
  USING (auth.uid() = user_id);

CREATE OR REPLACE FUNCTION public.touch_course_visit(
  p_course_id uuid,
  p_source text DEFAULT 'course'
)
RETURNS void
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
  ...
  INSERT INTO public.user_course_visits (user_id, course_id, last_visited_at, visit_count, last_source)
  VALUES (v_user_id, p_course_id, now(), 1, COALESCE(NULLIF(TRIM(p_source), ''), 'course'))
  ON CONFLICT (user_id, course_id)
  DO UPDATE
  SET
    last_visited_at = now(),
    visit_count = public.user_course_visits.visit_count + 1,
    last_source = COALESCE(NULLIF(TRIM(EXCLUDED.last_source), ''), public.user_course_visits.last_source),
    updated_at = now();
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.touch_course_visit(uuid, text) TO authenticated;
